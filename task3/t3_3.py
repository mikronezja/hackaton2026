...
def load_raw() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Build dtype map only for columns that actually exist
    ...
    # Normalise timestamp column to "Timedate" for the rest of the code
    if ts_col != "Timedate":
...
def aggregate_monthly(df: pd.DataFrame) -> pd.DataFrame:
    """One row per (deviceId, year, month)."""
    ...
    agg = df.groupby(["deviceId", "year", "month"]).agg(
        ...
        x1_mean  = ("x1", "mean"),
        x1_std   = ("x1", "std"),
        # target (NaN in val/test splits because x2 is withheld)
        x2_mean  = ("x2", "mean"),
        # quality
        n_readings = ("x2", "count"),
    ).reset_index()

    # Seasonal encoding – lets linear model see the annual cycle
...
def make_splits(monthly: pd.DataFrame):
    def _filter(ym_list):
        ...
        for y, m in ym_list:
            mask |= (monthly["year"] == y) & (monthly["month"] == m)
        return monthly[mask].copy()

    # Training: all rows where x2_mean is NOT NaN and within range
    train_mask = (
...
def build_historical_baseline(train: pd.DataFrame):
    """
    For each (deviceId, month): mean x2 observed in training data.
    Also compute global mean per month as fallback for unseen devices.
    """
    dev_month = (
        train.groupby(["deviceId", "month"])["x2_mean"]
        .mean()
        .rename("pred_hist")
        .reset_index()
    )
    global_month = (
        train.groupby("month")["x2_mean"]
        .mean()
        .rename("pred_hist_global")
        .reset_index()
    )
...
def predict_historical(df: pd.DataFrame, dev_month, global_month) -> np.ndarray:
    ...
    preds = merged["pred_hist"].fillna(merged["pred_hist_global"])
    return preds.values.astype("float32")
...
def train_lgbm(train: pd.DataFrame, val: pd.DataFrame):
    ...
    y_v  = val["x2_mean"].values  # NaN for validation split – handled below

    # Only use val rows that have ground truth for early stopping
    has_gt = ~np.isnan(y_v)
    ...
    if has_gt.sum() > 0:
        ...
        # No ground truth in val → train without early stopping
...
def optimise_weights(
    preds_val: np.ndarray,   # shape (N, 3)
    y_val: np.ndarray,       # ground truth, may contain NaN
) -> np.ndarray:
    """
    Minimise MAE on validation rows that have ground truth.
    ...
    def obj(w):
        ...
        w = w / w.sum()
        blended = pv @ w
        return np.mean(np.abs(blended - yv))
...
def build_forecast_frame(
    ...
    global_month: pd.DataFrame,
) -> pd.DataFrame:
    """
    Create one row per (deviceId, year, month) for all forecast months,
    filling missing features with device-level or global means.
    """
    device_ids = devices["deviceId"].unique()
    rows = []
    for y, m in FORECAST_MONTHS:
        for did in device_ids:
            rows.append({"deviceId": did, "year": y, "month": m})
    forecast = pd.DataFrame(rows)

    # Try to merge existing monthly aggregates (for val rows that may exist)
    ...
    # Fill missing feature values with device's training mean
    # Compute per-device mean from monthly (training period)
    dev_feat_means = monthly.groupby("deviceId")[BASE_FEATURES].mean()

    for feat in BASE_FEATURES:
        if feat in ("month_sin", "month_cos", "n_readings"):
            continue
        missing = forecast[feat].isna()
        if missing.any():
            forecast.loc[missing, feat] = forecast.loc[missing, "deviceId"].map(
                dev_feat_means[feat]
            )

    # Seasonal features always recomputed from target month
    ...
    forecast["n_readings"] = forecast["n_readings"].fillna(
        monthly["n_readings"].median()
    ).astype("int32")

    # Final fallback: global feature mean
    for feat in BASE_FEATURES:
        if feat in ("month_sin", "month_cos", "n_readings"):
            continue
...
def main():
    ...
    # --- Build full forecast frame (features for May–Oct 2025) ---
    forecast = build_forecast_frame(monthly, devices, global_month)
    print(f"Forecast frame shape: {forecast.shape}")

    # --- Val predictions (used for weight optimisation) ---
    # Val split: rows for May–Jun 2025 (x2 is withheld → NaN in 'val')
    # We predict on the forecast frame filtered to val months
    val_mask = forecast["month"].isin([5, 6]) & (forecast["year"] == 2025)
    val_fc   = forecast[val_mask].reset_index(drop=True)

    # Model A – Ridge
    model_a, _ = train_ridge(train, val_fc)
    pred_a_val  = predict_ridge(model_a, val_fc)

    # Model B – LightGBM
    model_b, _ = train_lgbm(train, val_fc)
    pred_b_val  = predict_lgbm(model_b, val_fc)

    # Model C – Historical
    pred_c_val  = predict_historical(val_fc, dev_month, global_month)

    # --- Optimise weights ---
    # ground truth for val (NaN because x2 withheld – but keep the call)
    ...
    # If val GT was available, report individual model MAEs
    ...
    final_pred = (preds_all @ w_opt).clip(0, 1)   # x2 is grid load index ∈ [0,1]
    ...
    # Sanity check: every device × 6 months must appear
    expected = len(devices) * 6
    assert len(submission) == expected, \
        f"Expected {expected} rows, got {len(submission)}"
