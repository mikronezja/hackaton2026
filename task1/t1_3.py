...
OUTPUT_PARQUET = "moje_predykcje_3.parquet"
OUTPUT_CSV = "moje_predykcje_3.csv" # Opcjonalnie, do łatwego podglądu
...
def main():
    ...
    fps_union = FeatureUnion([
        ("ecfp", ECFPFingerprint(fp_size=4096, radius=3, count=True, n_jobs=-1)),
        ("rdkit", RDKitFingerprint(fp_size=2048, n_jobs=-1)), # Dodany na podstawie dobrych praktyk z skfp
        ("tt", TopologicalTorsionFingerprint(n_jobs=-1)),
        ("estate", EStateFingerprint(n_jobs=-1))
    ])

    # Ulepszone parametry LGBM
    lgbm = LGBMClassifier(
        class_weight="balanced",
        n_jobs=1,          
        random_state=0,
        verbose=-1,
        n_estimators=1000,    # Więcej drzew...
        learning_rate=0.05,   # ...ale mniejszy krok (stabilniejsze uczenie)
        max_depth=8,          # Nieco głębsze drzewa
        num_leaves=63,        # Dostosowane do max_depth (2^k - 1)
        subsample=0.8,        # Stochastic Gradient Boosting
        colsample_bytree=0.8  # Losowanie cech dla każdego drzewa
    )
    ...
    try:
        ...
        raise FileExistsError(f"Nie udało się wczytać pliku Parquet z testami, błąd: {e}")
    ...
    # 7. Binaryzacja i przypisanie wyników do DataFrame
    ...
    # 8. Zapis wyników do pliku lokalnego zamiast wysyłania na serwer
    print(f"Zapisywanie predykcji do pliku {OUTPUT_PARQUET}...")
    submission_df.to_parquet(OUTPUT_PARQUET, index=False)
    
    print(f"Zapisywanie predykcji do pliku {OUTPUT_CSV} (do podglądu)...")
    submission_df.to_csv(OUTPUT_CSV, index=False)

    print("Gotowe! Wszystkie pliki zostały zapisane lokalnie.")
